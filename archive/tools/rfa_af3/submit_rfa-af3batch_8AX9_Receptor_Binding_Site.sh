#!/bin/bash
#SBATCH --job-name=rfa-af3batch_8AX9_Receptor_Binding_Site
#SBATCH --partition=gpu
#SBATCH -A ccl_lab_gpu
#SBATCH --gres=gpu:A30:1
#SBATCH --nodes=1 --ntasks=1 --cpus-per-task=4 --mem=48G
#SBATCH --array=1-5
#SBATCH --output=slurm_logs/rfa-af3batch_8AX9_Receptor_Binding_Site_%A_%a.out --error=slurm_logs/rfa-af3batch_8AX9_Receptor_Binding_Site_%A_%a.err

set -euo pipefail
echo "--- AF3 Stage2 (batch inference-only; binder has NO MSA) ---"
module load cuda/12.2.0 singularity

# ValueError: For devices with GPU compute capability 7.x (see https://developer.nvidia.com/cuda-gpus) the ENV XLA_FLAGS must include "--xla_disable_hlo_passes=custom-kernel-fusion-rewriter".
export XLA_FLAGS="--xla_disable_hlo_passes=custom-kernel-fusion-rewriter"

DESIGNS_LIST="/data/homezvol1/inagakit/Projects/initbinder/tools/rfa_af3/8AX9_Receptor_Binding_Site/designs.list"
MANIFEST_TSV="/data/homezvol1/inagakit/Projects/initbinder/tools/rfa_af3/8AX9_Receptor_Binding_Site/design_manifest.tsv"
...
SEED_DATA_JSON="/data/homezvol1/inagakit/Projects/initbinder/targets/8AX9/designs/Receptor_Binding_Site/rfa_af3/design_1_0_dldesign_0/design_1_0_dldesign_0/design_1_0_dldesign_0_data.json"

if [ ! -s "$SEED_DATA_JSON" ]; then
    echo "[ERR] Seed data.json not found: $SEED_DATA_JSON"
    echo "Run Stage 1 first: sbatch submit_rfa-af3seed_8AX9_Receptor_Binding_Site.sh"
    exit 2
fi

OUT_DIR="/data/homezvol1/inagakit/Projects/initbinder/targets/8AX9/designs/Receptor_Binding_Site/rfa_af3/$DESIGN_NAME"
mkdir -p "$OUT_DIR"
