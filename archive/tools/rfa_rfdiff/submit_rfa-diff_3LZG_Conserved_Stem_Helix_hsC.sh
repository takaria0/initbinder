#!/bin/bash
#SBATCH --job-name=rfa-diff_3LZG_Conserved_Stem_Helix_hsC
#SBATCH --partition=gpu
#SBATCH -A ccl_lab_gpu
#SBATCH --gres=gpu:A30:1
#SBATCH --nodes=1 --ntasks=1 --cpus-per-task=1 --mem=16G
#SBATCH --array=1-1
#SBATCH --output=slurm_logs/rfa-diff_3LZG_Conserved_Stem_Helix_hsC_%A_%a.out
#SBATCH --error=slurm_logs/rfa-diff_3LZG_Conserved_Stem_Helix_hsC_%A_%a.err
...
# Compute per-task allocation from TOTAL=4, PER_TASK=100
TOTAL=4
PER_TASK=100
...
--bind "/data/homezvol1/inagakit/Projects/initbinder/targets/3LZG/designs/Conserved_Stem_Helix/hs-C/rfa_rfdiff":/outputs \
--pwd /home \
