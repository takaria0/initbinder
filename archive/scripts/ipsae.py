...
if af3:
    ...
    #     with open(summary_file_path,'r') as file:
    #         data_summary=json.load(file)
    #         print(f'AF3 summary file: {summary_file_path}')
    #         print(f'AF3 iptm data: {data_summary["chain_pair_iptm"]}')
    #     af3_chain_pair_iptm_data=data_summary['chain_pair_iptm']
    #     print(f'AF3 unique_chains: {unique_chains}')
    #     for chain1 in unique_chains:
    #         nchain1=  ord(chain1) - ord('A')  # map A,B,C... to 0,1,2...
    #         for chain2 in unique_chains:
    #             print(f'chain1 {chain1} nchain1 {nchain1} chain2 {chain2}')
    #             if chain1 == chain2: continue
    #             nchain2=ord(chain2) - ord('A')
    ...
    if summary_file_path is not None and os.path.exists(summary_file_path):
        ...
        # 1) Determine AF3's chain order (try to read from JSON; otherwise fall back to your unique_chains order)
        # Some runners store fields like 'chain_order' or 'ordered_chain_ids'. Use them if present.
        chain_order = (
            data_summary.get('chain_order')
            or data_summary.get('ordered_chain_ids')
            or data_summary.get('chain_ids')
            or [str(c) for c in list(unique_chains)]  # fallback: preserve appearance order
        )
        chain_order = [str(c) for c in chain_order]
        print(f'AF3 chain_order used for indexing: {chain_order}')

        # 2) Build map from chain ID -> AF3 index
        chain_to_idx = {ch: i for i, ch in enumerate(chain_order)}

        # 3) Sanity check sizes
        n = len(af3_chain_pair_iptm_data)
        if n != len(chain_order):
            print(f'[WARN] ipTM matrix is {n}x{n} but chain_order has {len(chain_order)} items; '
                f'using first {n}.')
            chain_order = chain_order[:n]
            chain_to_idx = {ch: i for i, ch in enumerate(chain_order)}

        print(f'AF3 unique_chains (from CIF/input): {list(unique_chains)}')

        # 4) Fill your dict using the AF3 indices
        # make sure nested dict exists
        for ch in unique_chains:
            ch = str(ch)
            iptm_af3.setdefault(ch, {})
        ...
        for chain1 in unique_chains:
            ...
            for chain2 in unique_chains:
                ...
                if chain1 == chain2:
                    continue
                if chain1 not in chain_to_idx or chain2 not in chain_to_idx:
                    print(f'[WARN] {chain1} or {chain2} not in AF3 chain_to_idx; skipping.')
                    continue
                i = chain_to_idx[chain1]
                j = chain_to_idx[chain2]
                try:
                    iptm_val = af3_chain_pair_iptm_data[i][j]
                except IndexError:
                    print(f'[WARN] Index error for ({chain1},{chain2}) -> ({i},{j}); skipping.')
                    continue
                iptm_af3[chain1][chain2] = iptm_val
                print(f'[OK] ipTM[{chain1}->{chain2}] = {iptm_val} (i={i}, j={j})')
    else:
