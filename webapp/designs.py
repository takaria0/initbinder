...
def _schedule_assessment_autosync(
    ...
    *,
    ...
    When the design workflow schedules assessments we want the local results
    panel to populate automatically.  The assessment jobs can take a while, so
    this helper runs in the background waiting for the ``af3_rankings.tsv``
    artifact to appear on the cluster before invoking
    :meth:`ClusterClient.sync_assessments_back`.

    The polling interval is intentionally relaxed (default 60s) to avoid
    hammering the scheduler, and the wait stops after ``timeout_minutes`` to
    prevent threads from lingering forever in degenerate cases.
    """
    ...
    def _poll() -> None:
        ...
        while True:
            ...
            if match:
                ...
                message = f"[autosync] Waiting for assessment run {run_label} to appear on cluster…"
